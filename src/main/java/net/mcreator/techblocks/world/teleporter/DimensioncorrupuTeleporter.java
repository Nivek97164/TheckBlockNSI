...
public class DimensioncorrupuTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("dimensioncorrupu_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("dimensioncorrupu_portal",
				Sets.newHashSet(ImmutableSet.copyOf(TechblocksModBlocks.DIMENSIONCORRUPU_PORTAL.getStateDefinition().getPossibleStates())), 0, 1)
	...
	public DimensioncorrupuTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = TechblocksModBlocks.DIMENSIONCORRUPU_PORTAL.defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getPortalRepositioner(entity, blockpos1).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = DimensioncorrupuPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return DimensioncorrupuPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d,
